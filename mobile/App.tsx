...
export default function App() {
  const [session, setSession] = useState<any>(null); const [memberships, setMemberships] = useState<any[]>([]); const [church, setChurch] = useState<any>(null); const [loading, setLoading] = useState(() => Boolean(supabase));
  ...
  useEffect(() => { if (!supabase) return; let mounted = true; const applySession = (nextSession: any) => { if (!mounted) return; setSession(nextSession); void Promise.resolve().then(async () => { if (!mounted) return; if (nextSession) await reloadMemberships(); else { setMemberships([]); setChurch(null); } }); }; supabase.auth.getSession().then(({ data }) => { applySession(data.session); if (mounted) setLoading(false); }); const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => applySession(nextSession)); return () => { mounted = false; listener.subscription.unsubscribe(); }; }, [reloadMemberships]);
